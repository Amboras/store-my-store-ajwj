...
import { Search, ShoppingBag, User, Menu, X, LogIn, Dumbbell } from 'lucide-react'
        ...
        <div className="container-custom">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            <nav className="hidden lg:flex items-center gap-8">
              <Link href="/products" className="text-sm tracking-wide font-medium uppercase link-underline py-1" prefetch={true}>
                Shop All
              </Link>
              <Link href="/products/gripforce-silicone-resistance-ring" className="text-sm tracking-wide font-medium uppercase link-underline py-1" prefetch={true}>
                Resistance Rings
              </Link>
              <Link href="/products/gripforce-complete-training-set-all-5-levels" className="text-sm tracking-wide font-medium uppercase link-underline py-1" prefetch={true}>
                Training Sets
              </Link>
              {collections?.slice(0, 2).map((collection: any) => (
                <Link
                  ...
                  className="text-sm tracking-wide font-medium uppercase link-underline py-1"
                  prefetch={true}
                >
                  {collection.title}
                </Link>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                ...
                {itemCount > 0 && (
                  <span className="absolute top-0.5 right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-[hsl(4,85%,50%)] text-[10px] font-bold text-white">
                    {itemCount}
                  </span>
                )}
              </button>
          ...
          >
            <div className="flex items-center justify-between p-4 border-b">
              <div className="flex items-center gap-2">
                <div className="flex items-center justify-center w-7 h-7 bg-[hsl(4,85%,50%)] rounded-sm">
                  <Dumbbell className="h-3.5 w-3.5 text-white" strokeWidth={2.5} />
                </div>
                <span className="font-heading text-lg font-bold uppercase">GripForce</span>
              </div>
              <button
